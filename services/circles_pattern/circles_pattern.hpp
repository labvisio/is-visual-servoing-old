#ifndef __CIRCLES_PATTERN_HPP__
#define __CIRCLES_PATTERN_HPP__
...
bool compare_distance(PointDist const& x, PointDist const& y) {
...
PointsWithReference find_pattern(CompressedImage image) {
...
Pose get_pose(PointsWithReference P) {
  Pose pose;
  if (P.points.size() == 12) {
    pose.position.x = (P.points[4].x + P.points[7].x) / 2.0;
    pose.position.y = (P.points[4].y + P.points[7].y) / 2.0;
    pose.heading = std::atan2(P.points[2].y - P.points[0].y, P.points[2].x - P.points[0].x);
  } else {
